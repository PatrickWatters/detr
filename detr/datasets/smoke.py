...
def build(image_set, args):
    ...
    root2 = Path(args.label_path)  #separate (optional) root to label_path. will be same as root if not provided

    assert root.exists(), f'provided Smoke path {root} does not exist' #**replaced Coco with Smoke
    mode = 'annotations of data2'
    ...
    if not oneTime:
        PATHS = {                   
            ...
            "train": (root / "train_images/", root2 / f'annotation_density_train.json'),             #changed path to just include root for validation images and json file for bounding boxes for validation
            "val": (root / "val_images/", root2 / f'annotation_density_val.json'),

        }
    else:
        PATHS = {                   
            "train": (root / "ImgT", root / f'ann_t.json'),         #changed path to just include root for training images and json file for bounding boxes for training
            "val": (root / "Images_v", root / f'annot_v.json'),             #changed path to just include root for validation images and json file for bounding boxes for validation
        } 
